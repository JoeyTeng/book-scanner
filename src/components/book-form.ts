...
import { aggregateBookData, searchBookByTitle } from '../modules/api/aggregator';
...
export class BookForm {
  ...
  private attachEventListeners(): void {
    ...
    this.modalElement?.querySelector('#btn-refresh-isbn')?.addEventListener('click', () => {
      void this.handleRefreshFromAPI('isbn');
    });

    this.modalElement?.querySelector('#btn-refresh-title')?.addEventListener('click', () => {
      void this.handleRefreshFromAPI('title');
    });
    ...
    this.modalElement?.querySelector('#btn-smart-paste')?.addEventListener('click', () => {
      ...
      const button = this.modalElement?.querySelector('#btn-smart-paste') as HTMLButtonElement;
      ...
      try {
        ...
        // Fill form fields
        if (parsed.isbn)
          (this.modalElement?.querySelector('#input-isbn') as HTMLInputElement).value = parsed.isbn;
        if (parsed.title)
          (this.modalElement?.querySelector('#input-title') as HTMLInputElement).value =
            parsed.title;
        if (parsed.author)
          (this.modalElement?.querySelector('#input-author') as HTMLInputElement).value =
            parsed.author;
        if (parsed.publisher)
          (this.modalElement?.querySelector('#input-publisher') as HTMLInputElement).value =
            parsed.publisher;
        if (parsed.publishDate)
          (this.modalElement?.querySelector('#input-publish-date') as HTMLInputElement).value =
            parsed.publishDate;
        if (parsed.cover)
          (this.modalElement?.querySelector('#input-cover') as HTMLInputElement).value =
            parsed.cover;
        if (parsed.notes)
          (this.modalElement?.querySelector('#input-notes') as HTMLTextAreaElement).value =
            parsed.notes;
    ...
    this.modalElement?.querySelector('#btn-llm-parse')?.addEventListener('click', () => {
      void (async () => {
        const textarea = this.modalElement?.querySelector(
          '#smart-paste-input'
        ) as HTMLTextAreaElement;
        ...
        const button = this.modalElement?.querySelector('#btn-llm-parse') as HTMLButtonElement;
        const originalText = button.textContent;
        button.disabled = true;
        ...
        try {
          ...
          alert('Failed to parse with LLM. Please check your API configuration.');
        } finally {
          button.disabled = false;
          button.textContent = originalText;
        }
    ...
    // Manual LLM button
    this.modalElement?.querySelector('#btn-manual-llm')?.addEventListener('click', () => {
      const textarea = this.modalElement?.querySelector(
        '#smart-paste-input'
      ) as HTMLTextAreaElement;
    ...
    this.modalElement?.querySelector('#btn-search-by-title')?.addEventListener('click', () => {
  ...
  private showRefreshSelectionUI(results: BookDataSource[]): void {
    ...
    // Event listeners
    selectionModal.querySelector('#btn-close-selection')?.addEventListener('click', () => {
      selectionModal.remove();
    });

    selectionModal.querySelector('#btn-cancel-refresh')?.addEventListener('click', () => {
      selectionModal.remove();
    });

    selectionModal.querySelector('#btn-apply-refresh')?.addEventListener('click', () => {
      this.applyRefreshUpdates(selectionModal);
      selectionModal.remove();
    });
  }

  private renderRefreshField(field: keyof BookDataSource, results: BookDataSource[]): string {
      ...
      <div class="form-group">
        ...
        <div class="radio-group">
          <label>
            ...
            Keep current: <strong>${currentValueText || '(empty)'}</strong>
          </label>
          ${sources
            .map(
              (item) => `
